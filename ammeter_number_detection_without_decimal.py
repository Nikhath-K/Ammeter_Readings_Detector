...
def detect_numbers(frame):
    ...
    # Parse text to extract the integer part of the number
    try:
        # Convert to float to handle potential decimal numbers
        number = float(text.strip())
        # Return only the integer part
        return int(number)
    except ValueError:
        # Return None if the text cannot be converted to a number
        return None
    ...
    while True:
        ...
        if detected_number is not None and detected_number != previous_number:
            ...
            if previous_number is not None:
                if detected_number == previous_number + 1:  # Upcount sequence
                    led_upcount.off()
                    led_downcount.off()  # Ensure downcount LED is off in upcount sequence
                elif detected_number == previous_number - 1:  # Downcount sequence
                    led_upcount.off()  # Ensure upcount LED is off in downcount sequence
                    led_downcount.off()
                else:  # Missing number detected
                    if detected_number > previous_number + 1:  # Missing number in upcount
                        print(f"Missing number detected: {previous_number + 1}")
                        led_upcount.on()
                        led_downcount.off()  # Ensure downcount LED is off
                    elif detected_number < previous_number - 1:  # Missing number in downcount
                        print(f"Missing number detected: {previous_number - 1}")
                        led_downcount.on()
